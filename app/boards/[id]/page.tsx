...
export default function BoardPage() {
  ...
  function handleDragOver(event: DragOverEvent) {
    //Handling the UI changes when dragging over other items
    ...
    if (sourceColumn.id === targetColumn.id) {
      ...
      if (activeIndex !== overIndex) {
        setColumns((previousColumn: ColumnWithTasks[]) => {
          const newColumn = [...previousColumn];
          const column = newColumn.find(
  ...
  async function handleDragEnd(event: DragEndEvent) {
    //Handling the backend updates when drag ends
    const { active, over } = event;
    if (!over) return;
  ...
  const filteredColumn = columns.map((column) => ({
    ...
    tasks: column.tasks.filter((task) => {
      ...
      // If none of the filters excluded the task, include it
      return true;
    }),
  }));
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-50">
        ...
        <Dialog open={isEditingTitle} onOpenChange={setIsEditingTitle}>
          <DialogContent className="mx-auto w-[95vw] max-w-[450px]">
            <DialogHeader>
        ...
        lg:[&::-webkit-scrollbar-track]:bg-gray-100 lg:[&::-webkit-scrollbar-thumb]:bg-gray-300 lg:[&::-webkit-scrollbar-thumb]:rounded-full
        space-y-4 lg:space-y-0"
            >
              {filteredColumn.map((column, key) => (
                <DroppableColumn
                  key={key}
                  column={column}
    ...
    // 5.38 Next task is to add filter tasks
  );
}
